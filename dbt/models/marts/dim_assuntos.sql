...
explodido as (
    select codigo_assunto, nome_assunto
    from base
    lateral view inline(
        transform(assuntos_raw, x -> from_json(x, 'struct<codigo:bigint,nome:string>'))
    ) t as codigo_assunto, nome_assunto
    where codigo_assunto is not null
)
select
    {{ dbt_utils.generate_surrogate_key(['codigo_assunto']) }} as sk_assunto,
    codigo_assunto,
    min(nome_assunto) as nome_assunto    -- ← um único nome por código
from explodido
group by codigo_assunto